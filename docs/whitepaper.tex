...
% TODO: Define 'commands' in this section

%\subsection{Why did we decide to model speech using a fixed grammar?}
To reduce cognitive load, we focused on creating a concise and fixed grammar, meaning there is only one way to say a command. While speech 
recognition can be used for a conversation style of programming, we wanted to mimic the standard idea that computer languages have rigid grammars. 
...
We use a parser to map interface commands to commands in the grammar. The parser
extracts information on which action to execute and white blocks to manipulate
if necessary, and passes the structured information to the workspace controller.
% TODO This sentence is a bit vague
...
% TODO: Find out why we didn't use CMU's Sphinx speech recognition.
...
We perform speech recognition using the Google Speech API in Google Chrome. The API provides us with what it recognizes as the user's speech. We chose this option over CMU's Sphinx for unknown reasons 
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\For{$command$ in $commands$}
			...
			\EndIf
		\EndFor
		\If{$minEditDist \leq maxModification * length(rphon)$}
