...
% Can we get a citation to a definition?
...
beginning of speech recognition research.

The bulk of work aims to improve the productivity of professional developers
using IDEs. The first body of work here tends to specify its method to a
particular programming language. For example, \cite{Begel05programmingby}
creates a grammar for a language, Spoken Java, which is syntactically similar to
Java, with a goal of disambiguating natural language utterances.
\cite{desilets2006voicecode} uses a set of rules to trasncribe dictation results
into fragments of code.

The second group uses voice and other modalities, such as gesture and
