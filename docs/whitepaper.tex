...
\begin{itemize}
  ...
  \item Seperate - ``seperate block \uline{.5cm}''
...
We use a parser to map interface commands to commands in the grammar. The parser extracts information on which action to execute and white blocks to manipulate if necessary, and passes the structured information to the workspace controller.
...
We perform speech recognition using the Google Speech API in Google Chrome. The API provides us with what it recognizes as the user's speech. We chose this option over Carnegie Mellon University's Sphinx for unknown reasons 
